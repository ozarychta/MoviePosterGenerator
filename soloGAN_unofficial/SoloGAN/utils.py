...
# get_scheduler
# weights_init
def generateTargetLabel(s,dim):
    t= np.random.randint(0, dim)
    if(t!=s):
        return t
    else:
        return generateTargetLabel(s,dim)

# def label2onehot(label, dim):   #"""Convert label indices to one-hot vectors."""
#     out = torch.zeros(dim)
#     out[torch.tensor(label).long()] = 1
#     return out
