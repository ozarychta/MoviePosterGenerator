...
class Saver():
  def __init__(self, opts):
    ...
    print('directiory \n')
    print(self.display_dir,self.model_dir,self.image_dir)
    # make directory
  ...
  def write_display(self, total_it, model):
    if True: #(total_it + 1) % self.display_freq == 0:
      # write loss
  ...
  def write_img(self, ep, model):
    if (ep + 1) % self.img_save_freq == 0:
      assembled_images, label = model.assemble_outputs()
      img_filename = '%s/gen_%05d_%d.jpg' % (self.image_dir, ep, label)
      torchvision.utils.save_image(assembled_images / 2 + 0.5, img_filename, nrow=5)
    elif ep == -1:
  ...
  # save model
  def write_model(self, ep, total_it, model,lr):
    if (ep + 1) % self.model_save_freq == 0:
      print('--- save the model @ ep %d ---' % (ep))
      model.save('%s/%05d.pth' % (self.model_dir, ep), ep, total_it,lr)
    elif ep == -1:
      model.save('%s/last.pth' % self.model_dir, ep, total_it,lr)
